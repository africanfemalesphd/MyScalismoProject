...
object Metatarsal_4_Alignment {
  ...
  def main(args: Array[String])={
    ...
    val referenceMesh = MeshIO.readMesh(new File("data/CT_Images/CT2/CT2_4_Metatarsal.stl")).get
    val refLmks = LandmarkIO.readLandmarksJson[_3D](new File("data/CT_Images/CT2/CT2_4_Metatarsal.json")).get
    ...
    val AlignMeshes = (1 until 12).map { i: Int =>
      val mesh: TriangleMesh[_3D] = MeshIO.readMesh(new File("data/Objects/Left/Forefoot/4_Metatarsal/initial/triangle-meshes/CT"+i+"_4_Metatarsal.stl")).get
      //ui.show(targetGroup2, mesh, "Unaligned_FemurMeshes" + i)

      val UnalignedMeshLmks = LandmarkIO.readLandmarksJson[_3D](new File("data/Objects/Left/Forefoot/4_Metatarsal/initial/landmarks/CT"+i+"_4_Metatarsal.json")).get
      ...
      ui.show(targetGroup3, AlignedFemur, "Aligned_4_Metatarsal_" + i)

      //How to save the results in a folder
      MeshIO.writeMesh(AlignedFemur,new File("data/Objects/Left/Forefoot/4_Metatarsal/aligned/triangle-meshes/CT"+i+".stl"))

      LandmarkIO.writeLandmarksJson[_3D](TranslatedPointsToLmks,new File("data/Objects/Left/Forefoot/4_Metatarsal/aligned/landmarks/CT"+i+".json"))
