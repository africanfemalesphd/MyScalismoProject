...
object Cuneiform_Intermedial_Alignment {
  def main(args: Array[String])={
    ...
    val referenceMesh = MeshIO.readMesh(new File("data/CT_Images/CT2/CT2_Intermedial_Cuneiform.stl")).get
    val refLmks = LandmarkIO.readLandmarksJson[_3D](new File("data/CT_Images/CT2/CT2_Intermedial_Cuneiform.json")).get
    ...
    val AlignMeshes = (1 until 12).map { i: Int =>
      val mesh: TriangleMesh[_3D] = MeshIO.readMesh(new File("data/Objects/Left/Midfoot/Intermedial_Cuneiform/initial/triangle-meshes/CT"+i+"_Intermedial_Cuneiform.stl")).get
      //ui.show(targetGroup2, mesh, "Unaligned_FemurMeshes" + i)

      val UnalignedMeshLmks = LandmarkIO.readLandmarksJson[_3D](new File("data/Objects/Left/Midfoot/Intermedial_Cuneiform/initial/landmarks/CT"+i+"_Intermedial_Cuneiform.json")).get
      ...
      MeshIO.writeMesh(AlignedFemur,new File("data/Objects/Left/Midfoot/Intermedial_Cuneiform/aligned/triangle-meshes/CT"+i+".stl"))

      LandmarkIO.writeLandmarksJson[_3D](TranslatedPointsToLmks,new File("data/Objects/Left/Midfoot/Intermedial_Cuneiform/aligned/landmarks/CT"+i+".json"))
